...
async def handle_documents(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Get file from Telegram (memory mein rahega, PC mein nahi)
        file_obj = await file.get_file()
        
        # Download to memory (not to PC)
        import io
        file_data = io.BytesIO()
        await file_obj.download_to_memory(file_data)
        file_data.seek(0)
        
        # Save temporarily for upload (will be deleted after upload)
        file_path = f"downloads/{file_name}"
        os.makedirs("downloads", exist_ok=True)
        with open(file_path, 'wb') as f:
            f.write(file_data.getvalue())
        
        # REAL PROGRESS - Update step by step
